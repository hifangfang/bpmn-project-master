<template>
  <div class="process-panel__container">
    ...
    <el-collapse v-model="activeTab" v-else>
  ...
  methods: {
       ...
       init() {
          ...
          bpmnModeler.on('element.click', e => {
              if(e.element.type != 'bpmn:SequenceFlow'&&e.element.type != 'bpmn:Process'){
                  const newNode=document.getElementsByClassName("contextPad")[0]
                  var aDiv=this.getElementByAttr('div','data-container-id',e.element.id);//返回data-id为123456的div集合
                  const parentNode=aDiv[0];
                  const childNode=aDiv[0].childNodes[0]
                  if (parentNode){
                      this.$bus.$emit("changeElement",e.element)
                      parentNode.replaceChild(newNode,childNode);
                  }
                  this.selectedElements = [e.element]
                  this.getActiveElement(e);
              }else{
    ...
    getActiveElement(e) {
      ...
      this.initFormOnChanged(e.element.id)
      // 监听选择事件，修改当前激活的元素
