...
ContextPadProvider.prototype.getContextPadEntries = function(element) {
  ...
  // 只有点击列表中的元素才会产生的元素
  const contextPadArr=['bpmn:StartEvent', 'bpmn:Task', 'bpmn:BusinessRuleTask', 'bpmn:ExclusiveGateway', 'bpmn:DataObjectReference','bpmn:InclusiveGateway','bpmn:IntermediateThrowEvent']
  if (isAny(businessObject,contextPadArr )) {
    Object.assign(actions, {
      ...
      // // 'edit': editElement(),
      // 'replace':changeElement(),
      // 'delete': deleteElement()
    })
    console.log(actions)
  }
  // 某些节点只有删除和编辑-结束节点和线只有删除和编辑
  // const someArr= ['bpmn:EndEvent', 'bpmn:DataOutputAssociation','bpmn:TextAnnotation','bpmn:SequenceFlow']
